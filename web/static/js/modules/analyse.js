...
async function loadChartData() {
    ...
    try {
        const data = await fetchJson(`/api/data/history?sensor_id=${sensorId}&date=${date}&avg=${smoothing}`);
...
export function init(bridgeData) {
    ...
    if (!container.querySelector('#analysis-controls')) {
        const motionSensors = bridgeData.sensors.filter(s => s.type === "ZLLPresence");
                ...
                <div class="card-content form-grid">
                    <div class="form-group">
                        ...
                        <select id="sensor-select" class="styled-select">
                            ...
                            ${motionSensors.map(s => `<option value="${s.id}">${s.name}</option>`).join('')}
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="analysis-date">Datum:</label>
                        <input type="date" id="analysis-date" value="${new Date().toISOString().split('T')[0]}">
                    </div>
                    <div class="form-group">
        ...
        document.getElementById('btn-load-analysis').addEventListener('click', loadChartData);
    }
}
