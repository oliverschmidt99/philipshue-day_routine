...
function renderDeviceList(container, items, title) {
    ...
    if (items && items.length > 0) {
        content = `<ul>${items.map(item => `<li>${item.name} (ID: ${item.id})</li>`).join('')}</ul>`;
    } else {
...
export async function init() {
    ...
    if (container.innerHTML.trim() !== '' && !container.innerHTML.includes('wird noch implementiert')) {
        return;
    }

    container.innerHTML = '<div class="card"><div class="card-content" id="devices-content-wrapper"><p>Lade Gerätedaten...</p></div></div>';
