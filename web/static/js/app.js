        ...
        try {
            ...
            if (ips.length > 0) {
                ips.forEach(ip => {
                    ...
                    btn.type = 'button';
                    btn.className = 'w-full text-left p-3 bg-gray-100 rounded-lg hover:bg-blue-100 border-2 border-transparent focus:border-blue-500 focus:outline-none';
                    btn.textContent = ip;
                    btn.onclick = () => selectBridge(ip);
                    bridgeList.appendChild(btn);
                });
            } else {
                bridgeList.innerHTML = '<p class="text-gray-500">Keine Bridges automatisch gefunden.</p>';
            }
        } catch (e) {
...
function runMainApp() {
    ...
    const init = async () => {
        ...
        try {
            ...
            if (!configRes.ok) throw new Error(`Konfiguration konnte nicht geladen werden: ${configRes.statusText}`);
            config = await configRes.json();

            routinesContainer.innerHTML = `<p class="text-gray-500 text-center">Lade Bridge-Daten...</p>`;
            const [groupsRes, sensorsRes] = await Promise.all([
                fetch('/api/bridge/groups'),
                fetch('/api/bridge/sensors')
            ]);
            if (!groupsRes.ok) throw new Error(`Gruppen konnten nicht geladen werden: ${groupsRes.statusText}`);
            if (!sensorsRes.ok) throw new Error(`Sensoren konnten nicht geladen werden: ${sensorsRes.statusText}`);
            ...
            const errorMsg = `Fehler beim Initialisieren: ${error.message}. Ist der Server erreichbar?`;
            routinesContainer.innerHTML = `<p class="text-red-500 text-center font-semibold">${errorMsg}</p>`;
            ...
            <div class="p-6">
                ...
                <form id="form-scene" class="space-y-4">
                    ...
                    <div id="ct-controls" class="${isColorMode ? 'hidden' : ''}">
                        <label for="scene-ct" class="block text-sm font-medium text-gray-700">Farbtemperatur (Kalt ⟷ Warm)</label>
            ...
            <div class="p-6">
                ...
                <form id="form-routine" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Startzeit (Gesamt)</label>
                            <input type="time" id="routine-start-time" value="${String(routine.daily_time.H1).padStart(2, '0')}:${String(routine.daily_time.M1).padStart(2, '0')}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Endzeit (Gesamt)</label>
                            <input type="time" id="routine-end-time" value="${String(routine.daily_time.H2).padStart(2, '0')}:${String(routine.daily_time.M2).padStart(2, '0')}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                        </div>
                    </div>
            ...
            <div class="p-6">
                ...
                <form id="form-create-routine" class="space-y-4">
                        <div>
                            ...
                            <input type="text" id="new-routine-name" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" placeholder="z.B. Wohnzimmer bei Tag">
                        </div>
                    <div>
                        ...
                        <select id="new-routine-sensor" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
    ...
    const updateStatus = async () => {
        try {
            ...
            if (statusData && typeof statusData === 'object' && !Array.isArray(statusData)) {
                renderStatus(statusData.routines || []);
                renderSunTimes(statusData.sun_times || null);
            } else {
                renderStatus([]);
                renderSunTimes(null);
            }
    ...
    const setupAnalyseTab = () => {
        ...
        weekPicker.value = `${now.getFullYear()}-W${String(weekNumber).padStart(2, '0')}`;
        ...
        fetchDataBtn.addEventListener('click', () => {
            ...
            if (sensorId && date) {
                loadChartData(sensorId, period, date);
            } else {
                showToast("Bitte Sensor und Datum auswählen.", true);
            }
        });

        if (sensorSelect.value) {
            loadChartData(sensorSelect.value, 'day', today);
        }
    };

    const loadChartData = async (sensorId, period, date) => {
        try {
    ...
    const renderChart = (data, period, selectedDate) => {
        ...
        chartInstance = new Chart(ctx, {
            ...
            options: {
                ...
                scales: {
                    x: xAxisOptions,
                    y: { type: 'linear', display: true, position: 'left', title: { display: true, text: 'Helligkeit (lightlevel)' } },
                    y1: { type: 'linear', display: true, position: 'right', title: { display: true, text: 'Temperatur (°C)' }, grid: { drawOnChartArea: false } }
                }
            }
        });
    };
    ...
    const loadHelpContent = async () => {
        ...
        try {
            ...
            if (data.content) {
                ...
                const faqButtons = helpContainer.querySelectorAll('.faq-question');
                ...
                throw new Error("Hilfe-Inhalt ist leer oder fehlerhaft.");
            }
        } catch (error) {
            helpContainer.innerHTML = '<p class="text-red-500">Fehler beim Laden der Hilfe-Datei.</p>';
    ...
    const loadSettings = () => {
        ...
        document.getElementById('setting-longitude').value = location.longitude || '';
        document.getElementById('setting-hysteresis').value = settings.hysteresis_percent || 25;
        document.getElementById('setting-datalogger-interval').value = settings.datalogger_interval_minutes || 15;
        document.getElementById('setting-loglevel').value = settings.log_level || 'INFO';
    };
    ...
    const saveSettings = async () => {
        ...
        if (!newConfig.location) newConfig.location = {};
        newConfig.location.latitude = parseFloat(document.getElementById('setting-latitude').value);
        newConfig.location.longitude = parseFloat(document.getElementById('setting-longitude').value);
        if (!newConfig.global_settings) newConfig.global_settings = {};
        newConfig.global_settings.hysteresis_percent = parseInt(document.getElementById('setting-hysteresis').value);
        newConfig.global_settings.datalogger_interval_minutes = parseInt(document.getElementById('setting-datalogger-interval').value);
        newConfig.global_settings.log_level = document.getElementById('setting-loglevel').value;
        ...
        try {
            ...
            config = newConfig;
            showToast("Einstellungen gespeichert. Bitte starte die Anwendung neu, damit alle Änderungen wirksam werden.");
        } catch (error) {
            showToast(`Fehler: ${error.message}`, true);
        }
    };

    const systemAction = async (url, confirmMsg) => {
    ...
    const downloadBackup = () => {
        if (confirm('Möchtest du ein Backup der Konfiguration und Messdaten erstellen?')) {
    ...
    const setupEventListeners = () => {
        ...
        document.getElementById('btn-restart-app').addEventListener('click', () => systemAction('/api/system/restart', 'Möchtest du die Anwendung wirklich neu starten?'));
        document.getElementById('btn-backup').addEventListener('click', downloadBackup);
        document.getElementById('btn-new-series').addEventListener('click', () => systemAction('/api/database/new_series', 'Möchtest du wirklich eine neue Messreihe starten? Die alte wird archiviert.'));
        document.getElementById('btn-merge-series').addEventListener('click', () => systemAction('/api/database/merge_series', 'Möchtest du die neueste archivierte Messreihe mit der aktuellen zusammenführen?'));
        ...
        document.body.addEventListener('click', (e) => {
            ...
            const routineIndex = target.closest('[data-index]')?.dataset.index;
        ...
        const tabs = [
            ...
            { btn: 'tab-status', content: 'content-status' },
            ...
            { btn: 'tab-hilfe', content: 'content-hilfe', init: loadHelpContent }
        ];
