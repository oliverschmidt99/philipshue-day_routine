...
from Room import turn_on_groups, turn_off_groups
...
class sensor:
    ...
    def turn_off_after_motion(
        self, sensor_id, wait_time, group_ids, brightness, t_time
    ):
        new_wait_time = None
        if self.get_motion_sensor_status(sensor_id) == True:
            ...
            if (
                self.last_control_time >= time() - wait_time
            ):  # Compare wait_time with current_time
                print("bye")
