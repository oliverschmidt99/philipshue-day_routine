...
class Routine:
    ...
    STATE_OFF = "OFF"; STATE_MOTION = "MOTION"; STATE_BRIGHTNESS_ACTIVE = "BRIGHTNESS_ACTIVE"; STATE_RESET = "RESET"

    def __init__(self, name, room, sensor, routine_config, scenes, sun_times, log, global_settings):
        self.name = name; self.room = room; self.sensor = sensor; self.config = routine_config
        self.scenes = scenes; self.sun_times = sun_times; self.log = log; self.global_settings = global_settings
        self.enabled = self.config.get("enabled", True); self.time_span = self._create_time_span(self.config)
        self.state = self.STATE_OFF; self.last_motion_time = None
        self.is_brightness_control_active = False; self.do_not_disturb_active = False
    ...
    def _create_time_span(self, config):
        daily_time_conf = config.get("daily_time", {}); H1 = daily_time_conf.get("H1", 0); M1 = daily_time_conf.get("M1", 0)
        H2 = daily_time_conf.get("H2", 23); M2 = daily_time_conf.get("M2", 59)
    ...
    def _set_automation_state(self, state):
        ...
        self.last_automation_state = {k: v for k, v in state.items() if k in ['on', 'bri', 'ct', 'hue', 'sat']}

    def _check_do_not_disturb(self, period_config):
        if not period_config.get("do_not_disturb", False):
            if self.do_not_disturb_active:
                self.log.info(f"[{self.name}] 'Bitte nicht stören' ist für diesen Zeitraum nicht konfiguriert und wird zurückgesetzt.")
                self.do_not_disturb_active = False
            return False
        
        actual_state_on = self.room.is_any_light_on()
        if actual_state_on is None: return self.do_not_disturb_active

        # ===== NEUE REGEL: DnD zurücksetzen, wenn das Licht aus ist =====
        if not actual_state_on and self.do_not_disturb_active:
            self.log.info(f"[{self.name}] Alle Lichter sind aus. 'Bitte nicht stören' wird zurückgesetzt.")
            self.do_not_disturb_active = False
            return False # Blockade aufheben
        # ==============================================================

        if self.do_not_disturb_active:
            return True # Bereits aktiv, weiterhin blockieren
        ...
        if is_different:
            self.log.warning(f"[{self.name}] Manuelle Änderung erkannt! Erwartet: {self.last_automation_state}, Ist: {current_light_state}. 'Bitte nicht stören' wird aktiviert.")
    ...
    def _handle_brightness_control(self, period_config, current_period):
        if not self.sensor or not period_config.get("bri_check", False):
            if self.is_brightness_control_active: self.is_brightness_control_active = False
            return False
        ...
        turn_on_threshold = threshold - hysteresis_value; turn_off_threshold = threshold + hysteresis_value
        
        if light_level > turn_off_threshold:
            if self.is_brightness_control_active:
                self.log.info(f"[{self.name}] Helligkeit ({light_level}) > Schwelle ({turn_off_threshold}). Deaktiviere Licht.")
                self._set_automation_state({"on": False}); self.is_brightness_control_active = False
            return True
            ...
            if bri_ct: new_state["ct"] = bri_ct
            if not self.is_brightness_control_active:
                self.log.info(f"[{self.name}] Helligkeit ({light_level}) < Schwelle ({turn_on_threshold}). Starte Regelung -> Helligkeit: {bri}, CT: {bri_ct}")
    ...
    def run(self, now):
        if not self.enabled:
            if self.state != self.STATE_OFF:
                self._set_automation_state({"on": False}); self.state = self.STATE_OFF; self.do_not_disturb_active = False
            return
        current_period = self.time_span.get_current_period(now)
        if not current_period: return
        period_config = self.config.get(current_period)
        if not period_config: return
        ...
        if self.sensor and period_config.get("motion_check", False):
            if self.sensor.get_motion():
                if self.state != self.STATE_MOTION:
                    self.log.info(f"[{self.name}] Bewegung erkannt -> Szene: '{period_config['x_scene_name']}'.")
                    scene_to_set = self.scenes.get(period_config["x_scene_name"])
                    if scene_to_set: self._set_automation_state(scene_to_set.get_state())
                    self.state = self.STATE_MOTION; self.is_brightness_control_active = False
                self.last_motion_time = now; return
        ...
        if self.state != current_period and not self.is_brightness_control_active:
            self.log.info(f"---------- Zustands-Wechsel für '{self.name}' -> '{current_period}' ----------")
    ...
    def get_status(self):
        ...
        if self.do_not_disturb_active: last_scene_name = "Manuell (DnD aktiv)"
        elif self.state == self.STATE_MOTION and current_period_for_status and self.config.get(current_period_for_status):
            last_scene_name = self.config[current_period_for_status].get("x_scene_name", "N/A")
        elif self.is_brightness_control_active and current_period_for_status and self.config.get(current_period_for_status):
        ...
        return {
            "name": self.name, "enabled": self.enabled, "period": current_period_for_status,
