...
class Routine:
    ...
    def _handle_brightness_control(self, period_config, current_period):
        ...
        if light_level is None or light_level <= 0:
            return False
        ...
        if light_level < threshold_low:
            ...
            if not self.is_brightness_control_active:
                self.log.info(
                    f"[{self.name}] Starte Helligkeitsregelung. Sensor: {light_level} -> Lampe: {bri}"
                )
                base_scene_name = period_config["scene_name"]
                base_scene = self.scenes.get(base_scene_name)
                if not base_scene:
                    self.log.warning(
                        f"[{self.name}] Basis-Szene '{base_scene_name}' nicht gefunden."
                    )
                    return True
            ...
            self.log.info(
                f"[{self.name}] Helligkeit ({light_level}) über Ausschalt-Schwelle ({threshold_high}). Deaktiviere Licht."
            )
    ...
    def get_status(self):
        ...
        if (
            ...
            last_scene_name = f"Geregelt ({self.config[current_period_for_status].get('scene_name', 'N/A')})"
